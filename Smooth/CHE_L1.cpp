...
/** Computes the vertices in the star of a given vertex.*/
{
  ...
  if( !v_valid(v) ) { 
    if(DBG) cout << "CHE_L1::Vertex Star ERROR: invalid vertex id" << endl;
    star.push_back(INV); 
    return star; 
  }
...
/** Computes the vertices in the star of a given edge.*/
{
  ...
  if( !he_valid(h) ) 
  { 
    if(DBG) cout << "CHE_L1::Edge Star ERROR: invalid edge id" << endl;
    star.push_back(INV); 
    return star; 
  }
...
/** Computes the triangle incidents of a given triangle.*/
{
  ...
  if( !tr_valid(t) ) 
  { 
    if(DBG) cout << "CHE_L1::Triangle Star ERROR: invalid triangle id" << endl;
    star.push_back(INV); 
    return star; 
  }
  
  if( O(3*t)   != -1 ) star.push_back( trig(O(3*t))   );
  if( O(3*t+1) != -1 ) star.push_back( trig(O(3*t+1)) );
  if( O(3*t+2) != -1 ) star.push_back( trig(O(3*t+2)) );
...
/** Checks the mesh.*/
{
  ...
  if(3*ntrig() != static_cast<int>(_O.size()) ){ cout << "CHE_L1:: Error 3*ntrig()!= O.size" << endl; return;}
  if(  nvert() != static_cast<int>(_C.size()) ){ cout << "CHE_L1:: Error   nvert()!= C.size" << endl; return;}
  ...
  for(int i=0; i<3*ntrig(); ++i)
  {
    ...
    if( O(i)>=0 && O(O(i)) != i )
    {
      cout << "CHE_L1:: Error O(O(" << i <<")) != "<< i << endl;
    }
...
/** Gets a boundary compound. Sets _C*/
{
  // Stores Start && L1 time;
    ...
    L1_time = clock();
    //cout << "L1 load time:" << static_cast<float>(L1_time-start_time)/static_cast<float>(CLOCKS_PER_SEC) << endl;
	cout << endl;
}
/*--------------------------------------------------------------------------*/
