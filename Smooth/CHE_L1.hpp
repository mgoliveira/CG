...
public:
   ...
   /** \brief Tests if a vertex is on bound
     * \param v - const Vid  */
    virtual inline const bool v_bound( const  Vid v ) {
		vector<HEid> star = R_02(v);
		for(int i=0; i<(int)star.size(); ++i)
			if( O( vhe(v, star[i]) ) == -1 ) return true;
		return false; 
	}
	
//	inline const OKid faceok(const int nface ) const { return _faceok[nface] ; }
//	inline const void set_faceok( const int nface, const int w ) { _faceok[nface]=w ; }
...
public:
	...
	void persegue_curva (int he, int *faceok, float *obs);
