...
static Vertex V_INV( FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX,FLT_MAX, FLT_MAX, FLT_MAX,FLT_MAX, FLT_MAX, NULL, FLT_MAX );

/** \brief standard namespace definition*/
using namespace std;
...
/** \brief CHE_L0 Structure
  ...
  * mesh (vector _G) and the indices of each 
  * triangle (vector _V).*/
class CHE_L0
{
protected:
  ...
  /** \brief Vertices table 
    * 
    * Stores the index of the vertices 
...
public:
    ...
    /** \brief Access to the half--edge of vertex v in triangle t
      * \param const Vid v*/
	inline const HEid vhe( const Vid v, const TRid t ) {
		if( !v_valid(v) || !tr_valid(t) ) exit(1);  
...
public:
    ...
    * \param v - const Vid  */
	...
	void NLaplace (int tipo);			/////////////////////////////////////////<---------------------------------- 
	float ILaplace (float *obs, Vertex P);
	float ILaplaceS (float *obs, Vertex P);
	
	void interpola_normal (Vertex P, Vertex v1, Vertex v2);
	void bissec(float *obs,Vertex P, Vertex v1, Vertex v2);
	void draw_silhouette_phong(float *obs);
	float norma(Vertex P);
//	void transf_sist (Vertex v0, Vertex v1, Vertex v2);
	void grad_ilumina(float *obs);
	float iluminacao(float *obs, Vertex P);
//	void Laplacian_Lines(float *obs);
	void busca_zero(Vertex P, Vertex Q, float liP, float liQ, float *p, float *GiP);
//	void persegue_curva (int he, float *faceok, float *obs);
	int busca_face_prima (int f, float *obs);
	
	
//	void desenha_superficie(float *obs);

public:
	/** \brief Changes the normal orientation*/
	void flip_normals () ;
	/** \brief Computes the normal of each triangle*/     //cada vertice, ne?
	void compute_normals () ;
	
	void normaliza_normais();
...
public:
	...
	virtual void draw_verts () ;
	/** \brief Draws the vertices normals with opengl*/
	void draw_normals () ;
	/** \brief Draws the vector field with opengl*/
	void draw_vf() ;

	/** \brief Draws the silhoutte */
	void draw_silhouette(float *obs);
