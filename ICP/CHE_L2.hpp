...
/** \brief Edge iterator */
typedef  map<HEid, Edge>::iterator Eit;

/** \brief Edge constant iterator */
typedef  map<HEid, Edge>::const_iterator Ecit;

static Edge E_INV( INT_MAX, INT_MAX );
...
public:
   ...
   inline const map<HEid, Edge> &EH() const { return _EH ; }
...
public:
   /** \brief Sets an edge of the model  
     * \param e  - const HEid 
     * \param ed - const HEid */
   inline void set_EH( const HEid e, const Edge ed ) { _EH[e] = ed; }
   ...
   /** \brief Sets a vertex invalid
     * \param v - const Vid  */
   virtual inline const void v_invalid( const  Vid v ){ if( v_valid(v) ){CHE_L0::v_invalid(v); _VH[v] = INV;} return; }
